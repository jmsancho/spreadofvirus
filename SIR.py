# -*- coding: utf-8 -*-
"""
Created on Sun Mar 29 10:04:58 2020
...
#
#Retu
def findcoef(N,I0,R0,stepB=0.1,maxB=1,stepG=1,maxG=14,minG=1,days=160,
             verbose=False):
    """
    ...
    for B in np.arange(initB,maxB,stepB):
        for G in np.arange(initG,maxG,stepG):
            ...
            ret = odeint(deriv, y0, t, args=(N, B, Gg))
            S, I, R = ret.T
...
# all observed data
sols = findcoef(N,I0,R0,stepB=0.01,minG=6, maxB=3)
best = mqe(sols,slovenia)

print('----------------------------------------------------\n')
print("SIR WITH BETA AND GAMMA FITTED CONSIDERING ALL OBSERVATIONS\n")
...
plot_series(I,slovenia,'all_inf_v_real_SIR')

#plot series of projection of I,S and R
plot_curves(I,S,R,t,'all_SIR')
...
#plot series with real data and projection of infected
plot_series(I,slovenia,'last_inf_v_real_SIR')

#plot series of projection of I,S and R
plot_curves(I,S,R,t,'last_SIR')

plot_betas(test,slovenia,'betas_SIR')
today_pred=I[len(slovenia)]
